...
class GreenhouseController {
 ...
 private:
  ...
  String buildUiHead(const char *title) const {
    ...
    html += F(":root{color-scheme:dark;--bg:#071017;--bg-top:#101a24;--bg-edge:#050a0f;--panel:rgba(12,18,28,.90);--panel-soft:rgba(19,29,42,.88);--panel-strong:#0f1c29;--text:#e8f0f5;--muted:#8fa6b5;--line:rgba(129,165,178,.16);--line-strong:rgba(129,165,178,.28);--accent:#59d7a0;--accent-strong:#39b685;--accent-soft:rgba(89,215,160,.14);--accent-alt:#75c9ff;--ok:#72d98f;--warn:#e4c065;--danger:#f0818e;--shadow:0 26px 64px rgba(0,0,0,.36);--hero-a:#0f1d2a;--hero-b:#143346;--hero-c:#0f2d28;}html[data-theme='light']{color-scheme:light;--bg:#edf2f6;--bg-top:#f8fbfd;--bg-edge:#dfe8ef;--panel:rgba(255,255,255,.90);--panel-soft:rgba(243,248,251,.96);--panel-strong:#ffffff;--text:#15232c;--muted:#607483;--line:rgba(73,104,120,.16);--line-strong:rgba(73,104,120,.28);--accent:#1f8c68;--accent-strong:#187354;--accent-soft:rgba(31,140,104,.10);--accent-alt:#2e89c7;--ok:#2e8b57;--warn:#a06d14;--danger:#b64a57;--shadow:0 22px 52px rgba(17,39,51,.12);--hero-a:#183546;--hero-b:#20556b;--hero-c:#18463c;}*{box-sizing:border-box;}html,body{min-height:100%;scroll-behavior:smooth;}body{margin:0;font-family:'Segoe UI Variable Text','Segoe UI',system-ui,-apple-system,BlinkMacSystemFont,sans-serif;background:radial-gradient(circle at top,var(--bg-top) 0,var(--bg) 54%,var(--bg-edge) 100%);color:var(--text);font-size:15px;letter-spacing:.01em;}body:before{content:'';position:fixed;inset:0;background:linear-gradient(140deg,rgba(117,201,255,.08),transparent 28%),linear-gradient(220deg,rgba(89,215,160,.08),transparent 34%),radial-gradient(circle at 18% 16%,rgba(89,215,160,.10),transparent 18%);pointer-events:none;}main{position:relative;max-width:1180px;margin:0 auto;padding:22px 16px 48px;}section{background:var(--panel);backdrop-filter:blur(14px);border:1px solid var(--line);border-radius:22px;padding:18px;margin-bottom:16px;box-shadow:var(--shadow);}h1,h2{margin:0 0 10px;line-height:1.05;letter-spacing:-.02em;}h1{font-size:2.15rem;font-weight:800;}h2{font-size:1.15rem;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);}h3{margin:0 0 8px;color:var(--text);font-size:1rem;}p{line-height:1.6;margin:0 0 10px;}a{color:var(--accent-alt);text-decoration:none;}a:hover{text-decoration:underline;}code{background:rgba(117,201,255,.10);padding:2px 6px;border-radius:6px;}.theme-shell{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px;padding:12px 14px;border:1px solid var(--line);border-radius:18px;background:rgba(7,13,20,.52);backdrop-filter:blur(10px);}html[data-theme='light'] .theme-shell{background:rgba(255,255,255,.74);} .app-brand{display:flex;flex-direction:column;gap:3px;} .app-brand strong{font-size:.94rem;letter-spacing:.08em;text-transform:uppercase;} .app-brand span{font-size:.82rem;color:var(--muted);} .theme-switcher{display:inline-flex;align-items:center;gap:10px;padding:8px 10px;border-radius:999px;background:rgba(10,16,24,.42);border:1px solid var(--line);backdrop-filter:blur(10px);} html[data-theme='light'] .theme-switcher{background:rgba(255,255,255,.78);} .theme-switcher label{margin:0;font-size:.76rem;letter-spacing:.10em;text-transform:uppercase;color:var(--muted);font-weight:700;} .theme-switcher select{width:auto;min-width:110px;margin:0;padding:8px 32px 8px 12px;border-radius:999px;} .page-nav{display:flex;flex-wrap:wrap;gap:8px;margin:0 0 14px;padding:0;list-style:none;} .page-nav a{display:inline-flex;align-items:center;padding:9px 12px;border-radius:999px;background:rgba(13,20,30,.58);border:1px solid var(--line);color:var(--text);font-size:.83rem;font-weight:700;letter-spacing:.04em;text-transform:uppercase;} html[data-theme='light'] .page-nav a{background:rgba(255,255,255,.82);} .page-nav a:hover{text-decoration:none;border-color:var(--line-strong);background:var(--accent-soft);} .hero{background:linear-gradient(145deg,var(--hero-a),var(--hero-b) 58%,var(--hero-c));border:none;color:#eef8fb;overflow:hidden;position:relative;padding:22px;} .hero:after{content:'';position:absolute;inset:auto -12% -42% auto;width:360px;height:360px;background:radial-gradient(circle,rgba(117,201,255,.18),transparent 62%);pointer-events:none;} .hero h1,.hero strong{color:#f7fcff;} .hero p{color:#d6e6ee;max-width:66ch;} .hero-grid{display:grid;grid-template-columns:minmax(0,1.8fr) minmax(280px,1fr);gap:18px;align-items:start;} .kicker{display:inline-flex;align-items:center;padding:6px 10px;border-radius:999px;border:1px solid rgba(232,241,244,.14);background:rgba(232,241,244,.08);font-size:.74rem;letter-spacing:.12em;text-transform:uppercase;margin-bottom:12px;font-weight:800;} .grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:12px;} .card{background:var(--panel-soft);border-radius:18px;padding:14px 15px;border:1px solid var(--line);position:relative;overflow:hidden;} .card:before{content:'';position:absolute;inset:0 auto 0 0;width:3px;background:linear-gradient(180deg,var(--accent-alt),transparent 60%);opacity:.8;} .card strong{font-weight:800;} .card > strong:first-child{display:block;font-size:.78rem;letter-spacing:.09em;text-transform:uppercase;color:var(--muted);margin-bottom:7px;} .hero .card{background:rgba(232,241,244,.10);border-color:rgba(232,241,244,.14);color:#f4fbff;} .hero .card:before{background:linear-gradient(180deg,rgba(117,201,255,.85),transparent 60%);} .hero .card strong{display:block;font-size:1.7rem;line-height:1.05;margin-top:6px;} .hero .card span{display:block;font-size:.74rem;letter-spacing:.11em;text-transform:uppercase;color:#d6e7ef;font-weight:800;}label{display:block;font-weight:800;margin:10px 0 6px;font-size:.85rem;letter-spacing:.03em;}input,textarea,select{width:100%;padding:10px 12px;border:1px solid rgba(129,165,178,.22);border-radius:12px;box-sizing:border-box;font:inherit;background:rgba(8,14,21,.68);color:var(--text);} html[data-theme='light'] input,html[data-theme='light'] textarea,html[data-theme='light'] select{background:#fff;}textarea{min-height:88px;resize:vertical;}button{margin-top:12px;padding:10px 14px;border:none;border-radius:12px;background:linear-gradient(135deg,var(--accent),var(--accent-strong));color:#06120f;font-weight:800;cursor:pointer;letter-spacing:.02em;}button.secondary{background:linear-gradient(135deg,#395264,#567488);color:#eef6fb;} html[data-theme='light'] button.secondary{color:#fff;} .action-row{display:flex;flex-wrap:wrap;gap:10px;margin-top:12px;} .action-row button{margin-top:0;} .muted{color:var(--muted);} .notice{background:linear-gradient(135deg,rgba(117,201,255,.08),rgba(89,215,160,.06));border:1px solid rgba(117,201,255,.12);border-left:5px solid var(--accent-alt);border-radius:16px;padding:12px 14px;margin:12px 0;} .status-note{min-height:1.4em;margin-top:10px;font-size:.92rem;} .pill-row{display:flex;flex-wrap:wrap;gap:10px;margin:12px 0 0;} .pill{display:inline-flex;align-items:center;padding:7px 12px;border-radius:999px;background:var(--accent-soft);border:1px solid rgba(89,215,160,.18);font-size:.82rem;font-weight:800;letter-spacing:.04em;color:var(--text);} .pill.ok{background:rgba(114,217,143,.16);color:var(--ok);border-color:rgba(114,217,143,.24);} .pill.warn{background:rgba(228,192,101,.15);color:var(--warn);border-color:rgba(228,192,101,.22);} .pill.danger{background:rgba(240,129,142,.14);color:var(--danger);border-color:rgba(240,129,142,.24);} .section-intro{margin-top:-2px;margin-bottom:12px;max-width:66ch;font-size:.96rem;} .metric-strip{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-top:18px;} .message-wrap{max-width:760px;margin:0 auto;} .message-card{padding:24px;} .message-card h1{margin-bottom:10px;} @media (max-width:860px){.hero-grid{grid-template-columns:1fr;}.theme-shell{align-items:flex-start;flex-direction:column;}} @media (max-width:640px){main{padding:14px 12px 36px;}section,.hero{padding:16px;border-radius:18px;}.theme-shell{justify-content:stretch;}.theme-switcher{width:100%;justify-content:space-between;}.page-nav a{width:100%;justify-content:center;}} </style></head><body><main>");
    return html;
  }
