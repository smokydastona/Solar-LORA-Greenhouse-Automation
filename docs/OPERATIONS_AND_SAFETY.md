...
- The OLED drops into its configured idle protection mode after about 1 minute of normal idle time, or about 3 minutes while safe mode is active, to reduce burn-in. Firmware defaults now blank the screen completely. The local dashboard can switch that idle mode to a dim animated screensaver if you prefer motion over a fully dark panel. That animated mode now rotates through fireflies over grass, a plant growth cycle, parallax hills with a drifting moon, and seasonal particle scenes. Wake the OLED with any local mode button or another mode-changing action before treating the display as faulty.
